...
class Agent:
    ...
    def compute_new_velocity(self):
        ...
        for i in range(len(self.agent_neighbors_)):
            ...
            #check if the other agent is in the angle of vision
            relative_angle = angle_between_vectors(self.face_orientation_, relativePosition)
            if abs(relative_angle) > self.angle_of_vision:
                continue
